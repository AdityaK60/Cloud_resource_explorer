...
import { resourceExplorerPlugin } from './plugin';
...
describe('plugin', () => {
  it('should create and read TODO items', async () => {
    const { server } = await startTestBackend({
      features: [resourceExplorerPlugin],
    });

    await request(server).get('/api/resource-explorer/todos').expect(200, {
      items: [],
    });

    const createRes = await request(server)
      .post('/api/resource-explorer/todos')
      .send({ title: 'My Todo' });
    ...
    await request(server)
      .get('/api/resource-explorer/todos')
      .expect(200, {
        items: [createdTodoItem],
      });

    await request(server)
      .get(`/api/resource-explorer/todos/${createdTodoItem.id}`)
