...
import { EC2Client, StartInstancesCommand, StopInstancesCommand, RebootInstancesCommand, DescribeInstancesCommand, DescribeInstancesCommandOutput } from '@aws-sdk/client-ec2';
import {
  ResourceExplorerRequest,
  ResourceExplorerResponse,
  NormalizedInstanceData,
  CcdResourceExplorerConfig,
} from './types';
...
export async function createRouter(
  ...
  const pluginConfig = config.getOptionalConfig('ccdResourceExplorer');
  
  // Validate that plugin configuration exists
  if (!pluginConfig) {
    logger.warn(
      'CCD Resource Explorer plugin configuration is missing. ' +
      'Please add the "ccdResourceExplorer" section to your app-config.yaml file. ' +
  ...
  const apiConfig: CcdResourceExplorerConfig = {
    aws: {
      ...
      rds: {
        url: pluginConfig?.getOptionalString('aws.rds.url') || '',
        actionUrl: pluginConfig?.getOptionalString('aws.rds.actionUrl') || '',
        timeout: pluginConfig?.getOptionalNumber('aws.rds.timeout') || 30000,
      },
    },
    azure: {
      vm: {
        url: pluginConfig?.getOptionalString('azure.vm.url') || '',
        actionUrl: pluginConfig?.getOptionalString('azure.vm.actionUrl') || '',
        token: pluginConfig?.getOptionalString('azure.vm.token'),
        timeout: pluginConfig?.getOptionalNumber('azure.vm.timeout') || 30000,
      },
    },
    gcp: {
      compute: {
        url: pluginConfig?.getOptionalString('gcp.compute.url') || '',
        actionUrl: pluginConfig?.getOptionalString('gcp.compute.actionUrl') || '',
        token: pluginConfig?.getOptionalString('gcp.compute.token') || '',
        timeout: pluginConfig?.getOptionalNumber('gcp.compute.timeout') || 30000,
      },
      projects: {
        url: pluginConfig?.getOptionalString('gcp.projects.url') || '',
        token: pluginConfig?.getOptionalString('gcp.projects.token') || '',
        timeout: pluginConfig?.getOptionalNumber('gcp.projects.timeout') || 30000,
      },
    },
  };

  logger.info('CCD Resource Explorer plugin configured successfully');
  // Make apiConfig globally accessible for handler functions
  (globalThis as any).apiConfig = apiConfig;

  // Health check endpoint: GET /api/ccd-resource-explorer/health
  router.get('/health', (_, response) => {
    logger.info('Health check endpoint called');
    response.json({ status: 'ok' });
  });

  // Main endpoint to fetch cloud resources: POST /api/ccd-resource-explorer/resources
